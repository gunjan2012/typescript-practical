...
import { calcInput, eularButtons, numericValues, plusMinus, equalBtn, ceilFloor, powerButtons, deleteBtn, clearBtn, btnDeg, secondBtn, factorialBtn, secondPannelButtons, } from "./modules/utils.js";
// display the value of button when clicked
eularButtons.forEach((btn) => {
    btn.addEventListener("click", (e) => {
        let eularSign = e.target.innerText;
        switch (eularSign) {
            case "π":
...
let solveEq = () => {
    let valueByuser = calcInput.value;
    let numInput = valueByuser.slice(3);
    let angle = Number(((Number(numInput) * Math.PI) / 180).toFixed(4));
    let trignoFunc = valueByuser.slice(0, 3);
    let otherFunc = valueByuser.slice(0, 2);
    let numericVal = Number(valueByuser.slice(2));
    let regexNum = valueByuser.replace(/^0+/, "");
...
// ceil-floor button
ceilFloor.forEach((btn) => {
    btn.addEventListener("click", (e) => {
        if (e.target.value == "ceil") {
...
powerButtons.forEach((btn) => {
    btn.addEventListener("click", (e) => {
        let insVal = e.target.value;
        let disVal = Number(calcInput.value);
        switch (insVal) {
            case "oneByx":
                calcInput.value += "1/";
                break;
...
deleteBtn.addEventListener("click", () => calcInput.value = calcInput.value.slice(0, calcInput.value.length - 1));
// factorial function
let factEval = () => {
    let n = Number(calcInput.value);
    let answer = 1;
    if (n == 0 || n == 1) {
...
let secondFunc = () => {
    for (let i = 0; i < secondPannelButtons.length; i++) {
        const singleBtn = secondPannelButtons[i];
